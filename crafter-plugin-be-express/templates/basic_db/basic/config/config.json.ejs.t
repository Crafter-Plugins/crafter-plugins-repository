...
module.exports = {
  "development": {
    ...
    "<%= database.properties.name %>": {
      ...
      "host": "<% if (deploymentStackPlugins.includes('KubernetesManifest')) {-%><%=database.properties.name.toLowerCase() + '-service.' + blueprintName.toLowerCase() + '.svc.cluster.local' %><%} else { -%><%= 'localhost' %><%} -%>",
      "dialect": "postgres",
