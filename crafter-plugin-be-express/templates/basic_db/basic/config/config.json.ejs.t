...
module.exports = {
  "development": {
    ...
    "<%= database.properties.name %>": {
      ...
      "host": "<% if (deploymentStackPlugins.includes('KubernetesManifest')) {-%><%=database.properties.name.toLowerCase() + '.' + blueprintName.toLowerCase() + '-service.svc.cluster.local' %><%} else { -%><%= 'localhost' %><%} -%>",
      "dialect": "postgres",
